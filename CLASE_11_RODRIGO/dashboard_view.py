...
class DashboardView(ft.Container):
    ...
    def mostrar_tabla(self, nombre_tabla):
        ...
        if nombre_tabla =="Horarios_Base":
            horarios_vista = HorariosView(self.page, volver_atras=lambda: self.cambiar_vista(DashboardView(self.page, self.cambiar_vista)))
            self.cambiar_vista(horarios_vista)
        if nombre_tabla =="Cursos":
            cursos_vista = CursosView(self.page, volver_atras=lambda: self.cambiar_vista(DashboardView(self.page, self.cambiar_vista)))
            self.cambiar_vista(cursos_vista)
        if nombre_tabla == "Especialidades":
            especialidades_vista = EspecialidadesView(self.page, volver_atras=lambda: self.cambiar_vista(DashboardView(self.page, self.cambiar_vista)))
            self.cambiar_vista(especialidades_vista)
        else:
            dlg = ft.AlertDialog(
